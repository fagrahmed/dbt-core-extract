...
{{ config(
    ...
    pre_hook=[
        "{% if target.schema == 'dbt-dimensions' and source('dbt-dimensions', 'wallets_stg_exp') is not none %}TRUNCATE TABLE {{ source('dbt-dimensions', 'wallets_stg_exp') }};{% endif %}"
    ]
)}}
...
SELECT
    ...
    final.wallet_registredat_local,
