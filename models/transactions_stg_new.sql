...
{{ config(
    ...
    pre_hook=[
        "{% if target.schema == 'dbt-dimensions' and source('dbt-dimensions', 'inc_transactions_stg_new') is not none %}TRUNCATE TABLE {{ source('dbt-dimensions', 'inc_transactions_stg_new') }};{% endif %}"
    ]
)}}
